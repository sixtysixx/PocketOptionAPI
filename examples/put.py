...
async def main():
    ...
    try:
        ...
        order = await client.place_order(
            asset=symbol, amount=amount, direction=direction, duration=60
        )
        print(
            f"Order placed successfully: {order.order_id}, amount: {order.amount}, direction: {order.direction}, duration: {order.duration}"
        )
